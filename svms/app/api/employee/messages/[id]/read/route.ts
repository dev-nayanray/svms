...
import { markConversationRead } from "@/lib/services/message-cases";
...
export async function PATCH(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    const { scope, userId } = await resolveScope();
    const { id } = await ctx.params;
    const result = await markConversationRead(scope, id, userId);
    return ok(result);
  } catch (err) {
